 ...
 *      close, ctime, open, lseek, read, stat, timeisdigit
 *
      ...
      if (isfp->gcg_subformat == FORMAT_UNKNOWN)
        ...
        for (i=0; i < gcg_table_size; i++)
          if (isfp->gcg_subformat == gcg_table[i].format)
            break;
        s = (i < gcg_table_size ? gcg_table[i].ident : const_cast<char*>("GCG"));
      }
      break;
  ...
  while ((status = gi_getline(&line, &end, 1)) &&
    ...
    case 'L':
      if (strncmp(line, "LOCUS", 5) == 0) {
        if (allflag || flag == SEQINFO_IDLIST)
          add_id(&info, (isfp->db_idprefix ? isfp->db_idprefix :
	  const_cast<char*> ("gb")),
                 line+12, line+22);
    ...
    case 'P':
      if (toupper(line[1]) == 'I' && toupper(line[2]) == 'D' &&
        ...
        add_id(&info, (line[0] == 'N' ? const_cast<char*> ("nid") :
	const_cast<char*> ("pid")), line+12, end);
      break;
  ...
  while ((status = gi_getline(&line, &end, 0)) &&
      ...
      if (line[1] == 'D' && 
        ...
        if (allflag || flag == SEQINFO_IDLIST) {
          if (isfp->db_idprefix != NULL)
            ...
            if (count == 3 && period)
              prefix = (mystreq(t-3, 'E', "EPD;") ? const_cast<char*> ("epd") :
	      const_cast<char*> ("embl"));
    ...
    case 'P':
      if (line[1] == 'I' && (allflag || flag == SEQINFO_IDLIST))
        add_id(&info, (line[0] == 'N' ? const_cast<char*> ("nid") :
	const_cast<char*> ("pid")), line+5, end);
      break;
...
static int nbrf_getinfo(INTSEQFILE *isfp, char *entry, int len, int flag)
{
  ...
  if (allflag || flag == SEQINFO_IDLIST) {
    ...
    while (s < end && !isspace(*s)) {
      for (t=s; s < end && !isspace(*s) && *s != '|'; s++) ;
      add_id(&info, (isfp->db_idprefix ? isfp->db_idprefix : const_cast<char*>
      ("oth")), t, s);
      if (*s == '|') s++;
    }
  }
...
static int blastout_getinfo(INTSEQFILE *isfp, char *entry, int len, int flag)
{
  ...
  if (allflag || flag == SEQINFO_FRAGMENT || flag == SEQINFO_STARTPOS) {
    pattern = (isfp->entry_seqno == 1 ? const_cast<char*> ("\nQuery:") :
    const_cast<char*> ("\nSbjct:"));
                         ...
                         int flag)
{
   ...
   * a history line noting the annotation.
   */
  putline(fp, 0, 5, newcomment, 80, (xxflag ? const_cast<char*> ("XX   ") :
  const_cast<char*> ("CC   ")), 1);
  fputs((xxflag ? "\nXX   \nXX   " : "\nCC   \nCC   "), fp);
